...
const OptionsButton = props => {
    ...
    return (
        <>
            <IconButton
                id={`problem-type-options-button-${props.id}`}
                color="primary"
                aria-label="Опции"
                aria-controls={`problem-type-menu-${props.id}`}
                aria-haspopup="true"
            ...
            <Menu
                id={`problem-type-menu-${props.id}`}
                anchorEl={optionsButtonRef.current}
                open={menuOpen}
                onClose={() => setMenuOpen(!menuOpen)}
                MenuListProps={{
                    'aria-labelledby': `problem-type-options-button-${props.id}`,
                }}
            >
